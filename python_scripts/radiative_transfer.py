...
def i_sca(lamb, bc03):
    ...
    y_min = .18  # get better estimate of lower bound
    y_max = tau_f(lamb, 0)-.00001
    y = np.linspace(y_min, y_max, num_div)  # tau grid, 0 to tau(0)
    z_min = .01
    z_max = 1000
    z = np.linspace(.01, 1000, num_div)  # R grid
    v_min = -3
    v_max = 1000
...
for p in paths[:1]:
    ...
    num_div = 50
    y_min = 100
    y_max = 6*10**4  # lambda grid, 91 to 10**4 A
